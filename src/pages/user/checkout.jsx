...
function CheckoutPage() {
  ...
  useEffect(() => {
    const loadBalance = async () => {
      try {
        const response = await WalletService.fetchBalance();
        if (response.success) {
          setWalletBalance({
            availableBalance: response.data?.availableBalance || 0,
            onHoldBalance: response.data?.onHoldBalance || 0,
          });
          console.log(response);
        }
      } catch (err) {
        console.error("Failed to fetch wallet balance", err);
        toast.error("An error occured while fetching the user wallet balance");
        //setError("Could not load wallet balance. Please try again.");
      }
    };

    loadBalance(); // Fetch balance when the component mounts
  }, []);
  ...
  const hasAvailableItems =
    cartData &&
    cartData.shoppingCartItems.some(
      (item) =>
  ...
  const nextStep = async () => {
    ...
    const updatedHasAvailableItems =
      cartData &&
      cartData.shoppingCartItems.some(
        (item) =>
          item.status === "Selected" &&
    ...
    if (updatedHasAvailableItems) {
      ...
      toast.error(
        "You must have at least one available item in your cart to proceed."
      );
    }
  };
            ...
            {steps.map((s, index) => (
              <div
                ...
                className={`flex items-center ${
                  index <= step ? "text-orange-500" : "text-gray-400"
                }`}
              >
                <div
          ...
          {step === steps.length - 1 ? (
            ...
            <Button
              ...
              className={`bg-orange-500 hover:bg-orange-600 text-white ${
                !hasAvailableItems ? "opacity-50 cursor-not-allowed" : ""
              }`}
...
function CartStep({ cartData, updateItemStatus, fetchCartData }) {
...
function ShippingStep({ shippingAddress, setShippingAddress }) {
  return (
    <div className="space-y-4">
...
function ReviewStep({ cartData, shippingAddress, walletBalance }) {
  if (!cartData) {
    return <p>Loading...</p>;
  }

  const selectedItems = cartData.shoppingCartItems.filter(
    (item) =>
      item.status === "Selected" &&
      item.product.status.toLowerCase() === "available"
  );
  ...
  const merchandiseSubtotal = totalPrice;
  const totalPayment = merchandiseSubtotal + shippingFee;

  // Simulated wallet balance
  const balanceBeforePurchase = walletBalance.availableBalance;
  const balanceAfterPurchase = balanceBeforePurchase - totalPayment;
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between">
            <span>Balance before purchase</span>
            <span>${balanceBeforePurchase.toFixed(2)}</span>
          </div>
          <div className="flex justify-between">
            <span>Balance after purchase</span>
            <span>${balanceAfterPurchase.toFixed(2)}</span>
          </div>
        </div>
      </div>
    </div>
